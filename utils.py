...
def prepare_parser():
    ...
    parser.add_argument(
      ...
      help='Number of classes to generate, if not given, infer from dataset %(default)s)')
    ...
    ### Pretrained classifier stuff ###
    parser.add_argument(
        '--C_path', type=str, default="",
      help='Path to saved weights for Classifier (default: %(default)s)')
    parser.add_argument(
        '--C_arch', type=str, default="resnet50", choices=["resnet50", "densenet", "waterbird"],
      help='Architecture of Classifier (default: %(default)s)')
    parser.add_argument(
        '--C_n_classes', type=int, default=10,
      help='Number of classes for Classifier (default: %(default)s)')
    parser.add_argument(
        '--C_imagenet_norm', action='store_true', default=False,
      help='Normalize with imagenet mean and std before feeding to Classifier (default: %(default)s)')
    ...
    ### New class generation stuff ###
    parser.add_argument(
        '--D_second', action='store_true', default=False,
      help='Enable unconditional discriminator as a second branch? (default: %(default)s)')
    parser.add_argument(
        '--D_second_type', type=str, default="hinge",
      help='Loss type for unconditional discriminator ["hinge", "wgangp"]? (default: %(default)s)')
    parser.add_argument(
        '--D_only_uncond', action='store_true', default=False,
      help='Only unconditional discriminator? (default: %(default)s)')
    parser.add_argument(
        '--G_coeff', type=float, default=1.0,
      help='Loss coefficient for Generator (default: %(default)s)')
    parser.add_argument(
        '--G_new_coeff', type=float, default=1.0,
      help='Special loss coefficient for Generator on the new classes (default: %(default)s)')
    parser.add_argument(
        '--G_classmap', type=int, nargs="+", default=[],
      help='Corresponding class labels for Classifier (default: %(default)s)')
    parser.add_argument(
        '--D_coeff', type=float, default=1.0,
      help='Loss coefficient for main Discriminator (default: %(default)s)')
    parser.add_argument(
        '--D2_coeff', type=float, default=1.0,
      help='Loss coefficient for second Discriminator (default: %(default)s)')
    parser.add_argument(
        '--D2_reg_coeff', type=float, default=0.0,
      help='L2 regulation for second Discriminator logits when wgan-gp is used (default: %(default)s)')
    parser.add_argument(
        '--D_tilemix_ngrid', type=int, default=1,
      help='Number of tiles per dimension for TileMix before feeding to discriminator for unconditional branch (default: %(default)s means no mixing)')
    parser.add_argument(
        '--C_coeff', type=float, default=1.0,
      help='Loss coefficient for Classifier (default: %(default)s)')
    parser.add_argument(
        '--prob_clip', type=float, default=1.0,
      help='Clip maximum probability for Classifier (higher than this is considered no loss (default: %(default)s)')
    parser.add_argument(
        '--C_new_coeff', type=float, default=1.0,
      help='CrossEntropyLoss weight for the new classes (default: %(default)s)')
    parser.add_argument(
        '--new_class', type=int, default=-1,
      help='Label of the new class (default: %(default)s)')
    parser.add_argument(
        '--C_resize', type=int, default=0,
      help='Resize image before feeding to Classifier (default: %(default)s)')
    ...
    parser.add_argument(
      ...
      help='Number of samples to compute inception metrics with '
           '(default: %(default)s)')
    parser.add_argument(
        '--test_with_tf_inception', action="store_true", default=False,
      help='Compute metrics with tf inception weights instead of pytorch pretrained model (default: %(default)s)')
    parser.add_argument(
...
dset_dict = {'I32': dset.ImageFolder, 'I64': dset.ImageFolder, 
             ...
             'C10': dset.ImageFolder, 'C100': dset.CIFAR100,
             #'C10': dset.CIFAR10, 'C100': dset.CIFAR100}
             'C5_farthest4': dset.ImageFolder,
             'C5_closest4': dset.ImageFolder,
             'C9_wo_0': dset.ImageFolder,
             'C9_wo_1': dset.ImageFolder,
             'C9_wo_2': dset.ImageFolder,
             'C9_wo_3': dset.ImageFolder,
             'C9_wo_4': dset.ImageFolder,
             'C9_wo_5': dset.ImageFolder,
             'C9_wo_6': dset.ImageFolder,
             'C9_wo_7': dset.ImageFolder,
             'C9_wo_8': dset.ImageFolder,
             'C9_wo_9': dset.ImageFolder,
             'C10_wo_0': dset.ImageFolder,
             'C10_wo_1': dset.ImageFolder,
             'C10_wo_2': dset.ImageFolder,
             'C10_wo_3': dset.ImageFolder,
             'C10_wo_4': dset.ImageFolder,
             'C10_wo_5': dset.ImageFolder,
             'C10_wo_6': dset.ImageFolder,
             'C10_wo_7': dset.ImageFolder,
             'C10_wo_8': dset.ImageFolder,
             'C10_wo_9': dset.ImageFolder,
             'WB64_wo_0': dset.ImageFolder,
             'WB64_wo_1': dset.ImageFolder,
             'WB64': dset.ImageFolder,
             'WB128_wo_0': dset.ImageFolder,
             'WB128_wo_1': dset.ImageFolder,
             'WB128': dset.ImageFolder}
...
root_dict = {'I32': 'ImageNet', 'I32_hdf5': 'ILSVRC32.hdf5',
             ...
             'C10': 'cifar10/cifar10_all', 'C100': 'cifar',
             'C5_farthest4': 'cifar10/cifar10_farthest4',
             'C5_closest4': 'cifar10/cifar10_closest4',
             'C9_wo_0': 'cifar10/cifar9_wo_0',
             'C9_wo_1': 'cifar10/cifar9_wo_1',
             'C9_wo_2': 'cifar10/cifar9_wo_2',
             'C9_wo_3': 'cifar10/cifar9_wo_3',
             'C9_wo_4': 'cifar10/cifar9_wo_4',
             'C9_wo_5': 'cifar10/cifar9_wo_5',
             'C9_wo_6': 'cifar10/cifar9_wo_6',
             'C9_wo_7': 'cifar10/cifar9_wo_7',
             'C9_wo_8': 'cifar10/cifar9_wo_8',
             'C9_wo_9': 'cifar10/cifar9_wo_9',
             'C10_wo_0': 'cifar10/cifar10_wo_0',
             'C10_wo_1': 'cifar10/cifar10_wo_1',
             'C10_wo_2': 'cifar10/cifar10_wo_2',
             'C10_wo_3': 'cifar10/cifar10_wo_3',
             'C10_wo_4': 'cifar10/cifar10_wo_4',
             'C10_wo_5': 'cifar10/cifar10_wo_5',
             'C10_wo_6': 'cifar10/cifar10_wo_6',
             'C10_wo_7': 'cifar10/cifar10_wo_7',
             'C10_wo_8': 'cifar10/cifar10_wo_8',
             'C10_wo_9': 'cifar10/cifar10_wo_9',
             'WB64_wo_0': 'waterbird/water_bird_on_water',
             'WB64_wo_1': 'waterbird/land_bird_on_land',
             'WB64': 'waterbird/grouped',
             'WB128_wo_0': 'waterbird/water_bird_on_water',
             'WB128_wo_1': 'waterbird/land_bird_on_land',
             'WB128': 'waterbird/grouped'}
...
class TileMix(object):
    """Split a batch of images [N, C, H, W] into tiles, and randomly reassemble
    Args:
        ngrid (int): Number of tiles per dimension
    """
    def __init__(self, ngrid=2):
        self.ngrid = ngrid

    def __call__(self, data):
        """tile a batch of images of size [N, C, H, W], shuffle across all, and reassemble"""
        n_batch, n_channel, h, w = data.shape
        assert h == w, "must be square images for tile_and_shuffle operation!"
        assert h % self.ngrid == 0, "number of tiles per dimension is invalid!"
        tile_size = h // self.ngrid
        # unfold tensor
        unfold_data = F.unfold(data, tile_size, stride=tile_size)
        # shuffle the tiles
        idx = torch.randperm(unfold_data.shape[0]*unfold_data.shape[2])
        unfold_data = unfold_data.permute(1,0,2) \
                                .reshape(unfold_data.shape[1],-1)[:, idx] \
                                .reshape(unfold_data.shape[1], n_batch, self.ngrid*self.ngrid) \
                                .permute(1,0,2)
        # fold tensor back
        fold_data = F.fold(unfold_data, data.shape[2:], kernel_size=tile_size, stride=tile_size)
...
class RandomCropLongEdge(object):
    ...
    def __call__(self, img):
        ...
        return transforms.functional.crop(img, j, i, size[1], size[0])
        #BUG: PIL Image.size return (W, H), while torch crop expects (top, left) starting point and (H,W)
