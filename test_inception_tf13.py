...
def run(config):
    ...
    fname = "test_IS_horses.npz"
    print('loading %s ...'%fname)
    ims = np.load(fname)['x']
    import time
    t0 = time.time()
    preds, pool_activations = get_inception_score(list(ims.swapaxes(1,2).swapaxes(2,3)), splits=1)
    t1 = time.time()
    print('Saving pool to numpy file for FID calculations...')
    np.savez("tf_output.npz", **{"preds":preds, "pool3":pool_activations})
def main():
